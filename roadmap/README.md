...
| [challenge-recap-headless-permissions/](challenge-recap-headless-permissions/challenge-recap-headless-permissions.md) | 🚧 in-progress | high | @rabb1tl0ka | The `recap` skill fails silently in headless (`--print`) mode because Claude Code skips workspace trust and requires explicit Write permission rules that no tested format has matched yet. |
